...
def prepare(models, config, runtime):
    ...
    revision = HfApi().model_info(source["repository"], revision=source["revision"]).sha
    ...
    for name, spec in options["tokenizers"].items():
        ...
        revision = previous["resolved_revision"] if same_spec else HfApi().model_info(spec["repository"], revision=spec["revision"]).sha
        prefix = spec["subfolder"].strip("/")
        patterns = [prefix + "/*"] if prefix else ["*.json", "*.txt", "*.model", "*.jinja"]
        folder = Path(snapshot_download(spec["repository"], revision=revision, allow_patterns=patterns)) / prefix
        manifest["tokenizers"][name] = {p.relative_to(folder).as_posix(): sha256(p)
                                         for p in folder.rglob("*") if p.is_file()}
        manifest["tokenizer_sources"][name] = dict(spec, resolved_revision=revision, path=str(folder))
